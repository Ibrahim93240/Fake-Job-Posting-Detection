# Fake Job Posting Detection & Hiring Scam Analytics

## Project Overview
This project analyzes fake and real job postings to identify hiring scam patterns using Excel, Power Query, MySQL, Power BI, and Python Machine Learning.

## Tools Used
- Excel
- Power Query
- MySQL
- Power BI
- Python
- Pandas
- Scikit-learn
- NLTK
- GitHub

## Project Workflow
1. Data cleaning using Power Query
2. Exploratory analysis using Excel Pivot Tables
3. SQL business analysis using MySQL
4. Dashboard creation using Power BI
5. NLP text preprocessing using Python
6. Machine Learning model training for fake job prediction

## Key Insights
- Fake job postings represent around 4.84% of the dataset.
- Part-time jobs showed the highest fraud percentage.
- Oil & Energy and Accounting industries showed high scam risk.
- Administrative and finance-related functions were more scam-prone.
- “Data Entry”, “Payroll”, and “Work From Home” appeared frequently in fake postings.

## Machine Learning
A Logistic Regression model was trained using TF-IDF features extracted from job title and requirements text.

## Project Files
- `Dataset/` - cleaned dataset
- `Excel_Dashboard/` - Excel analysis and dashboard
- `SQL/` - SQL queries and views
- `PowerBI/` - Power BI dashboard
- `Python_ML_Model/` - notebook and trained model files

## Author
Ibrahim Khan